...
先人们为了解决这种难题，就将一个网站分成了许多部分，每个控件都被他们抽象为了一个对象，通过给对象添加属性来描述这个控件的外观、位置等属性，如果属性值是个函数，那么它就成了描述控件的方法。如此一来，开发团队的每个人都负责几个控件，最后将他们组织起来，就能成为一个网站，这种解决问题的思想，就是面向对象的细想，以这种思想为基础编码解决问题，就叫面向对象编程。
...
```
> 我们知道，当`===`用来处理数字和字符串时，是判断`===`两边数字或字符串的「值」是否相等。而用`===`来处理函数时则不是，因为函数没有所谓「值」，所以不能判断值是否相等，因此`===`用来处理函数时，则是判断两边是否为同一份函数，例如：
> 
> ```javascript
> // a 为函数 #1，b 为函数 #2
> var a = function () {}; // #1
...
> 
> // 将 a 赋值给 c，现在 c 的值是函数 #1
> var c = a;
> 
> // a 和 c 是同一份函数，他们都是 #1
> console.log(a === c); // true
> ```
> 当`===`作用于数组和对象的时候，它的行为也跟作用于函数一样，相关知识会在往后专门讲解。
...
3. 调用函数时`new`不`new`有什么区别？
4. 使用构造函数时，忘记加`new`有可能导致什么后果？
5. 什么是「实例」？
5. 当我们拿到一个实例的时候我们要如何判断它是被那个构造函数创建的？