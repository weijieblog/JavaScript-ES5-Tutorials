...
明白了变量的作用域，就可以很好的解决上面的疑问。变量的作用域可以理解成变量的作用范围，在 JavaScript 中，作用域是以函数为单位的，也就是说，我们在某个函数内部声明的变量，它的作用范围就在这个函数内部，换句话说，它只能在这个函数内部使用，外部其他函数都是访问不到的，例如下列代码中的`i`变量：
...
当一个变量是函数外部被声明的，我们就说，该变量存在于**全局作用域**，当变量在某个函数体中被声明的，那么，这个变量，就存在于该函数的本地作用域，例如下边的代码标识了全局作用域和`fn` `func`函数的本地作用域：
...
函数能够访问到自己本地作用域中的变量和全局作用域中的变量，不能访问其他函数作用域中的变量。函数的参数是函数本地作用域中的变量，它在函数被调用的一刻被赋值，外部作用域和其他函数的本地作用域，都访问不到它，了例如：
...
// 此时 func 中的 x1, x2, x3 是自己本地作用域中的，而不是 max 作用域中的
...
既然函数体中可以是任意合法的代码，那么，函数体内部也可以创建函数，然后在函数体内部被创建的函数的函数体中，又创建了函数，这个时候，就像俄罗斯套娃一样，如此一来，变量的作用域就变的有意思了。当一个函数体中的代码在执行时用到了某个变量，那么 JavaScript 会首先在这个函数的本地作用域中寻找这个变量，如果找到就直接使用，如果找不到，JavaScript 就到这个函数的父函数的作用域中找，如果找到就使用这个变量，如果再找不到，就到这个函数的爷爷函数的作用域中找，以此类推，一直找到全局作用域，找到就使用，找不到就只能是`undefined`了。例如：
...
```
当上边代码中最内层（第三层）函数运行时，在它的内部用到了`a, b, c, g, x`等等一连串的变量。其中`x`和`c`是在第三层作用域中找到的，`b`是在第二层作用域中找到的，`a`是在第一层作用域中找到的，`g`是在全局作用域中找到的。执行一下上边的`add`函数，验证我们的想法：
...
上边代码中最内层函数再运行时，JavaScript 会按照如下的链式顺序层层寻找这个函数所使用的变量，这种处理函数作用域的规则，被称为**作用域链**是不是很形象呢。

1. 第三层作用域 => 第二层作用域 => 第一层作用域 => 全局作用域

正因为 JavaScript 是从内层往外层寻找变量的，所以函数外部不能访问到函数本地作用域的变量，父级函数不能访问到子级函数本地作用域中的的变量。

### 立即执行的函数表达式

一开始，先人们在全局作用域中定义和使用变量。随着时代发展，当项目的规模越来越大，同一个项目会有多人参与，人人都使用全局作用域中的变量，导致全局作用域中变得拥挤不堪。出现了由于变量名重复而导致的种种 BUG，例如就像之前的一道练习题所示，小明新加入的代码中的变量`x`和小红之前定义的`x`变量名一致，意外导致了小红代码的崩溃。
...
这种表达式不用刻意去调用，它会立即执行，例如下面代码会直接在命令行中输出`123`：
...
同时，由于它是一个函数表达式，所以它有着自己的作用域，在它内部定义的变量，在全局作用域中访问不到，不会污染全局作用域。同时，外部也没法修改它本地作用域的变量。所以用它来包裹自己的代码，可以任意使用变量名而不用害怕会跟其他地方的代码冲突， 例如下边代码中用「立即执行的函数表达式」包裹小明新加入的代码，可以排除掉由于变量`x`冲突而导致的 BUG：
