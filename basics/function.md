...
我们知道函数体可以包含各种各样的代码，既然如此，函数体内部就能声明变量。如同如下代码所示，函数里有个`i`，函数外又有个`i`，代码中就有了两个`i`变量，那么谁是谁的谁呢，因此我们就需要了解**变量的作用域**和**变量访问的规则**。
...
总而言之，变量在哪个函数中被声明它就属于哪个函数的作用域，不在任何函数内部中被声明的变量就属于全局作用域，再举个例子，如下代码：
...
经过上边的例子，我相信大家能够很好的判断出哪个变量存在于哪个作用域中了，但有一点值得注意，某个函数的参数算作该函数本地作用域中的变量，例如：
...
函数的参数算作该函数本地作用域中的变量，那么上边代码的作用域以及变量如下图表示。
...
另外，由于函数内部还能再声明函数，所以，函数的作用域内部就能再包含作用域，作用域是可以嵌套的，如下代码所示，
...
通过上边的代码我们看到了在函数内部声明函数的情况，下图表示了这种情况的作用域关系，作用域会也根据函数的嵌套关系进行嵌套：
...
当这种情况发生，我们就说`second`是`third`的父级作用域，同样的，`first`是`second`的父级作用域，以此类推。

### 变量访问的规则

作用域的东西已经说的够多了，我们已经知道了如何划分作用域，也知道如何判断变量属于哪个作用域，但本小节开头的问题却还没解决，现在就来介绍 JavaScript 在寻找变量时的访问作用域的规则。
...
函数`func`用到了`i`，根据上边的流程图，JavaScript 会首先在这个函数的本地作用域中寻找`i`，找到了，这个`i`的值是`0`于是输出`0`，就不会再理会全局作用域中的`i`了。

JavaScript 变量访问的规则说白了就是个**由内向外**寻找变量的过程，即内层作用域可以访问到其外层作用域中的变量，但反过来就不行，这是之前举过的作用域层层嵌套的例子：
...
> 别忘了函数的参数算作该函数本地作用域中的变量

当上边代码中`third`函数运行时，用到了`a, b, c, g, x`等等一连串的变量，那么 JavaScript 会按照由内而外，也就是「third => second => first => 全局」的顺序寻找这些变量。然后在`third`本地作用域中找到`x`和`c`，在`second`的作用域中找到`b`，在`first`作用域中找到`a`，在全局作用域中找到`g`，如下图：
...
同时，由于它是一个函数，因此它有着自己的作用域，在它内部定义的变量，外部访问不到，于是便不会污染外部作用域。同时，由于 JavaScript 由内向外寻找变量的规则，外部也没法修改它本地作用域的变量。

用它来包裹自己的代码，就相当于有了一个独立的作用域，在这个作用域里使用变量名的时候便不用害怕它会跟其他地方的变量名冲突，例如下边代码中用「立即执行的函数表达式」包裹小明新加入的代码，可以避免由于变量`x`冲突而导致的 BUG：
...
u = (y + z) / x; // 小红代码原来的逻辑得以保留
 ```
> 有了「立即执行的函数表达式」我们就可以将自己的代码和其他代码隔离，降低相互影响的危险，所以在日常开发中，尽量避免直接使用全局作用域是个好习惯。
