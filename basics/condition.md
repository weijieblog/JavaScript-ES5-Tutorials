...
2. 如果后台取到的数据不为空，那么渲染页面
3. 如果该用户不是女性，那么不用给他推送化妆品广告
...
```
上边伪代码中有个很重要的概念，就是「条件」，条件在「条件语句」和往后介绍的「循环语句」中会被大量用到，「条件」通常情况下是**表达式**，此时「条件的结果」就是表达式的返回值，例如我们可以按照如下方式使用单一的`if`语句：
...
```
当表达式非常复杂时，我们可以将「条件」按照如下格式换行写，例如：
...
同时「条件」还可以是**变量**或者**值**，此时「条件的结果」就是变量持有的值或直接就是值本身，例如：
...
> }
> ```
>  这种`if...else...`语句稍后会介绍，出现这种意外也是一个提醒，提醒我们如果不是对结果非常有把握，否则不推荐在逻辑运算中混合使用布尔值和非布尔值。
...
```
if (条件) {
  当「条件」的结果为 true 时执行这个代码块的内容
} else {
  当「条件」的结果为 false 时执行这个代码块的内容
}
```
上述代码有如下需要注意的地方：
...
```
运行上面的代码，打印出了两个「欢迎来到电影大世界」。这是因为当程序运行到第八行时`user.age >= 18`返回的是`true`，然后程序运行了第九行所在的代码块的内容，打印了第一个「欢迎来到电影大世界」。然后程序运行到第十五行`user.gender === 'male'`返回了`true`，然后程序运行了第十六行所在的代码块的内容，打印了第二个「欢迎来到电影大世界」
...
```
if (条件1) {
  当「条件1」的结果为 true 时执行这个代码块的内容
} else if (条件2) {
  当「条件1」的结果为 false ，但是「条件2」的结果为 true 时执行这个代码块的内容
} else if (条件3) {
  当「条件1」「条件2」的结果都为 false， 但是「条件3」的结果为 true 时执行这个代码块的内容
} else {
  当所有上面所有「条件」的结果都为 false 时执行这个代码块的内容
}
```
上述代码有如下需要注意的地方：
...
10. 编写程序判断变量`num`是奇数还是偶数，如果是奇数给变脸`result`赋值为`'odd'`，如果是偶数给变脸`result`赋值为`'even'`，提示：判断奇偶可以使用取模运算。

10. 有长度不等的字符串`str1``str2``str3``str4`，编写程序找到其中长度最短的字符串将其赋值给变量`'theShortest'`，找到其中长度最长的字符串将其赋值给变量`'theLongest'`。
