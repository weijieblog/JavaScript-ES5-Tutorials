...
```
> 虽然代码块能单独使用，但它通常是伴随着某些语句一同出现，如`if`语句。
...
下列代码使用了 JavaScript 的`if...else...`语句描述一些常见的逻辑，比如某个电影网站，为了保护用户的安全，在用户访问网页的时候设计了如下逻辑：
...
然后我们就能用`if...else...`语句表述上边的逻辑，如下代码所示：
...
```
> 小练习：说说为什么运行上边的代码会打印两次「欢迎来到电影大世界」。

当我们运行上面的代码，会打印出两个「欢迎来到电影大世界」。网站的产品经理总觉得有点多余，于是改进了流程，如下图所示：
...
此时我们利用代码块可以包含代码块的设定，按照如下方式组合俩个`if...else...`就能表达上面流程图所表示的逻辑：
